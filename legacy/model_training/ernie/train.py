...
输出:
    ernie/saved_models/epoch_N/   （每轮一份 checkpoint）
"""
...
def train() -> None:
    ...
    logger.info(f'加载预训练模型: {ERNIE_PRETRAINED}')
    tokenizer = AutoTokenizer.from_pretrained(ERNIE_PRETRAINED)
    model = ERNIEEmotionClassifier.from_pretrained(ERNIE_PRETRAINED)
    model.to(device)
    ...
    ERNIE_CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        save_path = ERNIE_CHECKPOINT_DIR / f'epoch_{epoch + 1}'
